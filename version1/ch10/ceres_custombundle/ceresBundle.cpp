...
void BuildProblem(BALProblem* bal_problem, Problem* problem, const BundleParams& params)
{
    ...
    const int camera_block_size = bal_problem->camera_block_size();//9 旋转向量、平移 焦距 畸变
    double* points = bal_problem->mutable_points();  //环境中路标点个数
    double* cameras = bal_problem->mutable_cameras();//轨迹的pose个数
