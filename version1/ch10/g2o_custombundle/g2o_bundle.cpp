...
void BuildProblem(const BALProblem* bal_problem,     //参考书本P171,基本一样 
                  g2o::SparseOptimizer* optimizer, 
                  const BundleParams& params)
{
    const int num_points = bal_problem->num_points();
    const int num_cameras = bal_problem->num_cameras();
    const int camera_block_size = bal_problem->camera_block_size(); //9:相机点 0-2旋转向量 3-5平移 6 焦距 7-8畸变
    const int point_block_size = bal_problem->point_block_size();   //3 空间点坐标维度
    ...
    {
        ConstVectorRef temVecPoint(raw_points + point_block_size * j, point_block_size);//每个空间点坐标
...
{   
    ...
    if(params.linear_solver == "dense_schur" ){
        ...
        dynamic_cast<g2o::LinearSolverCholmod<BalBlockSolver::PoseMatrixType>* >
             (linearSolver)->setBlockOrdering(true);  // AMD ordering , only needed for sparse cholesky solver
    }
